...
export class MfaService {
  public async generateMFASetup(req: Request) {
    ...
    if (user.userPreferences.enable2FA) {
      return {
        success: false,
        message: "Multi facator configuration already enabled!",
      };
    }
    ...
    return {
      success: true,
      message: "Scan thr QR code or use the setup key!",
      secretKey: secretKey,
      qrImageUrl,
    };
  }
  ...
  public async verifyMFASetup(req: Request, code: string, secretKey: string) {
    ...
    return {
      success: true,
      message: "Multi facator configured successfully!",
      userPreferences: {
        enable2FA: user.userPreferences.enable2FA,
      },
    };
  ...
  public async revokeMFA(req: Request) {
    ...
    if (!user.userPreferences.enable2FA) {
      return {
        success: false,
        message: "Multi facator configuration is not enabled!",
        userPreferences: {
          enable2FA: user.userPreferences.enable2FA,
        },
      };
    ...
    return {
      success: true,
      message: "Multi facator un-configured successfully!",
      userPreferences: {
        enable2FA: user.userPreferences.enable2FA,
      },
    };
